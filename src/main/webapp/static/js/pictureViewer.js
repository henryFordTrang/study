;(function ($, document, window) {
    $.pictureViewer = function (options) {
        var pictureViewer_html =    '<div id="pictureViewer">' +
                                    ...
                                    '<div class="picture-content" id="picture-content-id">' +
        ...
        var changeImage = function changeImage(index) {
        	if($images[index].indexOf(".mp4") != -1 || $images[index].indexOf(".avi") != -1){
  			  $("#picture-content-id").html('<video class="cover" src="'+$images[index]+'" style="max-height:100% !important" controls="controls">您的浏览器不支持video播放</video>');
  		    }else{
  			  $("#picture-content-id").html('<img class="cover" src="'+$images[index]+'">');
  		    }
            $nowImageIndex = index;
            changeImageNum();
        };
